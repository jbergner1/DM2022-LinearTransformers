...
# ╔═╡ 0e74930e-ae6a-11ec-0076-e394657bf594
begin
	using Flux
	using Flux: onehot
	using Flux: gradient
...
# ╔═╡ 92f11b78-c14e-475e-a8ed-4312d730ed35
has_cuda()

# ╔═╡ 330b0912-7281-4f27-b0e7-0aef45414fee
# Configurator
begin
num_ite = 3000
batch_size = 8
...
# ╔═╡ 2c7fbc54-e2e8-4cc6-91fb-52c2cf6b11ef
# Labeling
begin
...
# ╔═╡ b781701a-f3c3-470e-bc87-51cf772b85fa
# function for generate training datas 
sample_data() = (d = map(string, rand(1:10, seq_len-2)); (d,d))

# ╔═╡ 91988449-8813-4e0e-9cb4-680f4ebde5d7
#function for adding start & end symbol

preprocess(x) = [startsym, x..., endsym]

# ╔═╡ c26f9b6b-0a33-43da-a2a1-f3e1aa7114ca
# use Vocabulary to encode the training data
begin
    @show sample_ex = preprocess.(sample_data())
    @show encoded_sample_ex = vocab(sample_ex[1]) 
end

# ╔═╡ 77958499-ba82-4df5-8624-108da62a589f
# test
sample = preprocess.(sample_data())

# ╔═╡ 979883d6-0208-43df-aed8-4fae674cd3da
# test
encoded_sample = vocab(sample[1])

# ╔═╡ e68e9252-e633-4f9f-97a1-d491aba3c6d2
#define a Word embedding layer which turn word index to word vector
embed = Embed(512, length(vocab)) |> gpu

# ╔═╡ 635858bd-9015-4206-bf94-a0bd7c8fb60e
#define a position embedding layer mentioned above
pe = PositionEmbedding(512) |> gpu

# ╔═╡ dc115d1f-30a7-4888-b4bd-e05498be7c6f
...
# ╔═╡ f6e55ca7-d455-467f-a702-3273e8cdc3a3
abstract type AbstractAttention end

# ╔═╡ 21ced7e2-4195-4a60-8295-b463c627a969
# elu(x) + 1 => nelu
begin
    function oftf(x, y)
        oftype(float(x), y)
...
# ╔═╡ ab712e6d-8083-4721-8b6b-7dff7b1656ab
# create embed training data
x = embedding(encoded_sample);

# ╔═╡ 3e3ed1c5-ddef-4490-a77c-fd4639a516d0
# new function splitHeads which reshape the 3D Tensor to 4D Tensor
function splitHeads(x, batch_size, head, depth)
    x = reshape(x, (batch_size, :, head, depth))
end

# ╔═╡ ee2f59a2-7f3d-4ada-abb5-cfc65b1bc878
# Using RAdam optimierer
opt = ADAM(1e-4)

# ╔═╡ c5600b54-8dc0-472d-8d10-35d6060097a3
#hier anstatt softmax linear
function linattention(query::A1,
    key::A2,
    value::A3,
    mask, future::Bool,
...
# ╔═╡ 96195029-b3e8-4562-ae4d-fcf2f35657cc
# modified MultiheadAttention from Transformers.jl
begin

	struct MultiheadAttention_mod{Q<:Dense, K<:Dense, V<:Dense, O<:Dense, DP<:Dropout} <: AbstractAttention
    head::Int
    future::Bool
    iqproj::Q
    ikproj::K
	...
	Flux.functor(mh::MultiheadAttention_mod) = (mh.iqproj, mh.ikproj, mh.ivproj, mh.oproj), m -> MultiheadAttention_mod(mh.head, mh.future, m..., mh.drop)

	MultiheadAttention_mod(head::Int,
                   is::Int,
                   hs::Int,
                   os::Int;
                   future::Bool=true, pdrop = 0.1) = MultiheadAttention_mod(head,
                                                                        future,
                                                                        Dense(is, hs*head),
                                                                        Dense(is, hs*head),
                                                                        Dense(is, hs*head),
...
function Base.show(io::IO, mh::MultiheadAttention_mod)
	...
	function (mh::MultiheadAttention_mod)(query::A1,
    key::A2,
    value::A3;
...
atten = linattention(ipq,ipk,ipv,
mask,
mh.future,
mh.drop,mh.head)
...
# ╔═╡ f278ecd7-f04e-41ab-bf2a-63b8b1ed8e40
abstract type AbstractTransformer end

# ╔═╡ a5527a11-a5c1-4636-a2f4-610fb046d05a
# Wrapper for two dense layer
...
# ╔═╡ 62ac9bf8-20f0-4d89-b6d2-cf3b78be9a6e
begin
	struct Transformer_mod{MA<:MultiheadAttention_mod, LA<:LayerNorm, P<:PwFFN, LP<:LayerNorm, DP<:Dropout} <: AbstractTransformer
    mh::MA
    mhn::LA
    pw::P
    pwn::LP
...
@functor Transformer_mod


########
#ganze Funktion von Transformer ändern
########
function Transformer_mod(size::Int, head::Int, ps::Int; future::Bool = true, act = relu, pdrop = 0.1)  
    rem(size, head) != 0 && error("size not divisible by head")
    Transformer1(size, head, div(size, head), ps;future=future, act=act, pdrop=pdrop)
end

Transformer_mod(size::Int, head::Int, hs::Int, ps::Int; future::Bool = true, act = relu, pdrop = 0.1) = Transformer_mod(
    MultiheadAttention_mod(head, size, hs, size; future=future, pdrop=pdrop),
    LayerNorm(size),
    PwFFN(size, ps, act),
    LayerNorm(size),
    Dropout(pdrop),     
...
function (t::Transformer_mod)(x::A, mask=nothing) where {T, N, A<:AbstractArray{T, N}}
    dropout = t.drop
...
function Base.show(io::IO, t::Transformer_mod) 
...
# ╔═╡ 6f9ddf62-a86e-4878-9c51-fc3b1446a2b2
begin
	struct TransformerDecoder_mod{MA<:MultiheadAttention_mod, LA<:LayerNorm,
        IMA<:MultiheadAttention_mod, ILA<:LayerNorm,
...
@functor TransformerDecoder_mod

function TransformerDecoder_mod(size::Int, head::Int, ps::Int; act = relu, pdrop = 0.1)
rem(size, head) != 0 && error("size not divisible by head")
TransformerDecoder_mod(size, head, div(size, head), ps; act=act, pdrop=pdrop)
end

TransformerDecoder_mod(size::Int, head::Int, hs::Int, ps::Int; act = relu, pdrop = 0.1) = TransformerDecoder_mod(
MultiheadAttention_mod(head, size, hs, size; future=false, pdrop=pdrop),
LayerNorm(size),
MultiheadAttention_mod(head, size, hs, size; future=true, pdrop=pdrop), 
LayerNorm(size),
PwFFN(size, ps, act),
LayerNorm(size),
Dropout(pdrop),
...
function (td::TransformerDecoder_mod)(x::AbstractArray{T,N}, m, mask=nothing) where {T,N}
dropout = td.drop
a = td.mh(x,x,x)
a = dropout(a)
res_a = x + a
...
function Base.show(io::IO, td::TransformerDecoder_mod)
...
# ╔═╡ 41e39936-7476-461d-8467-e63c92650e55
#define 2 layer of transformer

encode_t1 = Transformer_mod(512, 8, 64, 1024) |> gpu

# ╔═╡ b2756241-7a1b-4b81-be5b-dd9eccaf3031
encode_t2 = Transformer_mod(512, 8, 64, 1024) |> gpu

# ╔═╡ a08b4329-f605-4bf1-9d89-508d3b5d6048
encode_t3 = Transformer_mod(512, 8, 64, 1024) |> gpu

# ╔═╡ 1cf17a50-ea35-4c15-8049-587ce71fbd7e
encode_t4 = Transformer_mod(512, 8, 64, 1024) |> gpu

# ╔═╡ 4464b230-d4dc-46d0-b8df-1bd6e5c72ef4
#define 2 layer of transformer decoder

decode_t1 = TransformerDecoder_mod(512, 8, 64, 1024, act=nelu) |> gpu

# ╔═╡ de2ff083-f9d3-4e63-a0de-140444e07284
decode_t2 = TransformerDecoder_mod(512, 8, 64, 1024, act=nelu) |> gpu

# ╔═╡ 249ee6bf-8fb5-4a67-b62d-2186e3c7cc45
decode_t3 = TransformerDecoder_mod(512, 8, 64, 1024, act=nelu) |> gpu

# ╔═╡ 1ddfa0a6-9fce-4863-8781-3ec3e5c38b63
decode_t4 = TransformerDecoder_mod(512, 8, 64, 1024, act=nelu) |> gpu

# ╔═╡ e244d4f9-5699-49fe-a289-21b27b81aa63
#define the layer to get the final output probabilities

linear = Positionwise(Dense(512, length(vocab)), logsoftmax) |> gpu

# ╔═╡ 2b946f02-a2be-4e5d-b9ba-98fbf9f2373d
...
# ╔═╡ a2a4d30f-adf7-4cb8-8516-140a58ada87c
...
# ╔═╡ 4c828b34-7481-45bd-b5fc-20c73fec66e5
enc = encoder_forward(encoded_sample);

# ╔═╡ 56092834-9ac2-4d8e-aedd-75b719a73220
probs = decoder_forward(encoded_sample, enc);

# ╔═╡ f9d0190d-af6d-4d3b-952b-0647aa8b4d71
function smooth(et)
...
# ╔═╡ f1680d04-c6e2-4817-a27c-3494d4683752
Flux.@nograd smooth

# ╔═╡ d7c2ff8a-0472-4852-bf75-be159c9b70c1
...
function train!()
    ...
    losses =[]
    for i = 1:num_ite
      data = batched([sample_data() for i = 1:batch_size]) #create 32 random sample and batched
...
# ╔═╡ 239d525b-687a-4777-b1d0-e2bfdd441d17
# copy function
function copy(x)
...
# ╔═╡ edab4a0d-7090-45a2-a4b7-1f5d013bad5a
copy(map(string, rand(1:10, seq_len-2)))
